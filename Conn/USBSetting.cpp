                                   ...
                                   USBScanResult *usb_scan_result)
{
    ...
    if(prefer_com_port_.empty())
    {
        is_found = FindUSBDevice(usb_scan_arg, usb_scan_result, forbid_stop_);
    }
    else
    {
        if (QString::fromStdString(prefer_com_port_).toInt() > 0)
        {
            LOGI("Searching user specified com port COM%s",prefer_com_port_.c_str());
        }
        else {
            LOGI("Searching user specified com port %s",prefer_com_port_.c_str());
        }
        is_found = FindSpecialUSBDevice(usb_scan_arg, usb_scan_result, prefer_com_port_.c_str());
    }
